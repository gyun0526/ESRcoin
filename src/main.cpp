...
#if defined(NDEBUG)
# error "ESRcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x418e1ecc577ed9c1a4df0dd245df185102dd80e0fa87a8724b42e50a59c8b93a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ESRcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ESRcoin Signed Message:\n";
...
{
    // ESRcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    //nSubsidy >>= (nHeight / 840000); // ESRcoin: 840k blocks in ~4 years
...
static const int64 nTargetTimespan = 10 * 30 ; // ESRcoin: 5 m
static const int64 nTargetSpacing = 1 * 30; // ESRcoin: 30 s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ESRcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ESRcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x418e1ecc577ed9c1a4df0dd245df185102dd80e0fa87a8724b42e50a59c8b93a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Thu, 11 Jan 2018 00:41:59 GMT This is ESRCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xbd8c39d9e6ceb5a7ff6935da51892bc78aa8c5bb0c23ccb3dc64e3ade062137f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc7, 0xd3, 0xb5, 0xbd }; // ESRcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ESRcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ESRcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ESRcoinMiner(CWallet *pwallet)
{
    printf("ESRcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("esrcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ESRcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ESRcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ESRcoinMiner, pwallet));
}
